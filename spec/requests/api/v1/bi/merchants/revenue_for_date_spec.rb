...
describe "BI API - Merchant Revenue for Date" do
  context 'GET /api/v1/merchants/revenue?date=x' do
    ...
    let! (:m0i_invoiceitems) { create_list(:invoice_item, 3, invoice: m0_invoice, created_at: '2012-03-16 11:55:05')}

    let (:m1_invoice) { create(:invoice, merchant: merchants[1]) }
    let! (:m1i_invoiceitems_on_date) { create_list(:invoice_item, 3, invoice: m1_invoice, created_at: '2012-03-16 11:55:05')}
    let! (:m1i_invoiceitems_off_date) { create_list(:invoice_item, 3, invoice: m1_invoice)}
